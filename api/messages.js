// api/messages.js - Vercel serverless function for messages data
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'GET') {
            const result = await client.query('SELECT data FROM epic_messages ORDER BY created_at DESC');
            const messages = result.rows.map(r => r.data);
        ...
        if (req.method === 'PUT') {
            ...
            await client.query(
                'UPDATE epic_messages SET data = $1, updated_at = NOW() WHERE id = $2',
        ...
        if (req.method === 'DELETE') {
            ...
            await client.query('DELETE FROM epic_messages WHERE id = $1', [id]);
